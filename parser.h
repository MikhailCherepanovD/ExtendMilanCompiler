...
private:
    ...
    void enumeration(); //разбор блока перечисления
    ...
    bool checkAlreadyInitialized(const string&);// функция проверяет, была ли переменная уже проинициализированна. Если была: true
    int AddVariableWithoutChecking(const string &var);// Добавляет переменную в конец без проверки
